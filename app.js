...
function delayCheck(lat,lon) {
	...
	weathertest(lat,lon,function(probability){
		if (probability > 0.75) {
			delays[1] = 1;
			app.set('weatherStatus', 1);
			console.log("weather delay");
		} else { 
			delays[1] = 0;
			app.set('weatherStatus',0);
...
app.get('/arduinoCheck', function (req, res) {
	...
	var weatherStatus = req.app.get('weatherStatus');
	console.log("weatherStatus: " + weatherStatus);

	var trainStatus = req.app.get('trainStatus');
	console.log("statusTest: " + weatherStatus);
});
